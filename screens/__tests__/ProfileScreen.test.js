...
describe('ProfileEditScreen', () => {
  ...
  it('logs in successfully with credentials are valid', async () => {
    ...
    wrap.find('#usernameInput').simulate('changeText', TEST_BGG_USR)
    wrap.find('#passwordInput').simulate('changeText', TEST_BGG_PWD)

    await wrap.find('#submitButton').simulate('press')
  ...
  it('displays error messge when login fails', async () => {
    ...
    wrap.find('#usernameInput').simulate('changeText', 'someBadUser')
    wrap.find('#passwordInput').simulate('changeText', 'password')
